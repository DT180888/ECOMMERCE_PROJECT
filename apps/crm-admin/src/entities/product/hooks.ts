...
export const skuKeys = {
  all: ["skus"] as const,
  list: (params?: AdminSkuListParams) => ["skus", "list", params] as const,
};

export function useAdminSkuList<TData = PageRes<AdminSkuItem>>(
  params?: AdminSkuListParams,
  options?: Partial<UseQueryOptions<PageRes<AdminSkuItem>, unknown, TData, ReturnType<typeof skuKeys.list>>>
) {
  return useQuery<PageRes<AdminSkuItem>, unknown, TData, ReturnType<typeof skuKeys.list>>({
    queryKey: skuKeys.list(params),
    queryFn: () => listAllSkus(params),
    staleTime: 10_000,
    placeholderData: keepPreviousData,
    ...(options as any),
  });
...
export function useBulkUpdateSkuPrice() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (payload: BulkPriceUpdateReq) => bulkUpdateSkuPrice(payload),
    onSuccess: async () => {
...
export function useBulkUpdateSkuStock() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (payload: BulkStockUpdateReq) => bulkUpdateSkuStock(payload),
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: skuKeys.all });
    },
  });
...
export function useBulkUpdateSkuStatus() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (payload: BulkStatusUpdateReq) => bulkUpdateSkuStatus(payload),
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: skuKeys.all });
    },
  });
