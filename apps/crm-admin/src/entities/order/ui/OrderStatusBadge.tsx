...
export default function OrderStatusBadge({ status, className }: OrderStatusBadgeProps) {
  return (
    <span
      className={clsx(
        'inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold bg-background text-foreground shadow-neo-inset-sm select-none border-none shrink-0',
        className
      )}
    >
      <span className={clsx('w-1.5 h-1.5 rounded-full shrink-0 animate-pulse', dotColors[status] || 'bg-gray-500')} />
